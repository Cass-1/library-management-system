...
describe("Functional Tests for User Route", async () => {
    ...
    describe("userRoute GET requests", async () => {
        ...
        beforeAll(async () => {
            ...
            const response = await server.repository?.UserCollection?.insertOne(data);
            expect(response?.acknowledged).toBe(true);
            expect(response?.insertedId).toBe(GETExampleUser._id);
        })
        // remove the user from the database
        afterAll(async () => {
            const response = await server.repository?.UserCollection?.deleteOne({ _id: GETExampleUser._id });
            expect(response?.acknowledged).toBe(true);
            expect(response?.deletedCount).toBe(1);
        })

        it("gets a user successfully", async () => {
            const res = await request(server.App).get(`/user/${GETExampleUser._id}`);
    ...
    describe("POST Route Tests", async () => {
        afterAll(async () => {
            const response = await server.repository?.UserCollection?.deleteOne({ _id: POSTExampleUser._id });
            expect(response?.acknowledged).toBe(true);
            expect(response?.deletedCount).toBe(1)
        })
        it("creates a user successfully", async () => {
            var data = POSTExampleUser;
            const res = await request(server.App).post("/user/").send(data);
    ...
    describe("DELETE Route Tests", async () => {
        ...
        it("deletes a user successfully", async () => {
            const res = await request(server.App).delete(`/user/${DELETEExampleUser._id}`);
            expect(res.statusCode).toBe(200);
            expect(res.body.acknowledged).toBe(true);
        ...
        it("fails to delete a user because of a malformed request", async () => {
            const res = await request(server.App).delete("/user/aa");
            expect(res.statusCode).toBe(422);
            expect(res.body.message).toBe("Validation Chain Error");
        });
    })
    ...
    describe("PATCH Route Tests", async () => {
        ...
        // remove the user from the database
        afterAll(async () => {
            const response = await server.repository?.UserCollection?.deleteOne({ _id: PATCHExampleUser._id });
            expect(response?.acknowledged).toBe(true);
            expect(response?.deletedCount).toBe(1)
        })

        it("updates a user successfully", async () => {
            const response = await request(server.App).patch(`/user/${PATCHExampleUser._id}`).send({ age: 1000 });
            const getResponse = await request(server.App).get(`/user/${PATCHExampleUser._id}`);
        ...
        it("fails to update a user because user doesn't exist", async () => {
            const id = new ObjectId()
            const response = await request(server.App).patch(`/user/${id}`).send({ age: 1000 })
            expect(response.statusCode).toBe(200);
            expect(response.body.acknowledged).toBe(true);
        ...
        it("fails to update a user because of a malformed request", async () => {
            const response = await request(server.App).patch("/user/aaaaaa").send({ age: 1000 })
