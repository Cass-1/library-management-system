...
Figure out how to use DRM or another method to manage the distributed eBooks
...
Allow for members to renew (a limited number of times) and have an approval process that is automated based on if there are reservations, inter-library loans, or an active recall

#### 4.5 Inter-Library Loans

Haven't decided if this can be done by Members or have some request system for new books that we can then get through inter-library loans
...
#### 7.1 Proper Tagging of Inventory

Each item must have correct tags to enable searchability
...
Be able to add or remove books to the catalogue
...
- design the database with scalability in mind
  - avoid common anti-patterns (eg unbounded array)
